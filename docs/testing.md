...
bounded claim became `confirmed`; see [Platform and host evidence](platform-evidence.md). A future
model/CLI/OS combination remains unproven until that lane is executed; model availability from one
Cursor account is not a universal supported-model promise.
